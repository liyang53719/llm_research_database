...
input_dir=${1:-"${package_dir}/mmd"}
output_dir=${2:-"${package_dir}/png"}
...
mapfile -d '' mmd_files < <(
  find "$input_dir" -type f -name '*.mmd' -print0 | sort -z
)
...
mermaid_config="${temp_dir}/mermaid-config.json"
printf '%s\n' '{"maxTextSize": 5000000}' > "$mermaid_config"
...
for source_file in "${mmd_files[@]}"; do
  ...
  stem=${relative_path%.mmd}
  render_input=$source_file
  output_file="${output_dir}/${stem}.png"
  mkdir -p -- "$(dirname -- "$output_file")"

  if grep -Eq '^[[:space:]]*sequenceDiagram([[:space:]]*)$' "$source_file"; then
    render_input="${temp_dir}/input/${relative_path}"
    mkdir -p -- "$(dirname -- "$render_input")"
    awk '
      NR == 1 { sub(/^%%[[:space:]]+\{init:/, "%%{init:") }
      $0 !~ /^[[:space:]]*%%/ { gsub(/;/, "#59;") }
      { print }
    ' "$source_file" > "$render_input"
  elif head -n 1 "$source_file" | grep -Eq '^%%[[:space:]]+\{init:'; then
    render_input="${temp_dir}/input/${relative_path}"
    mkdir -p -- "$(dirname -- "$render_input")"
    sed '1s/^%%[[:space:]]\+{init:/%%{init:/' "$source_file" > "$render_input"
    echo "警告：${relative_path} 第 1 行存在 '%% {init'，使用临时修正版渲染。"
  fi

  printf '[%02d/%02d] %s ... ' \
    "$((success_count + failure_count + 1))" "${#mmd_files[@]}" "$relative_path"
